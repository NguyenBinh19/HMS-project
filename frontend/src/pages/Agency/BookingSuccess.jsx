...
import {
    Check, Copy, Home, Download, Wallet, MapPin,
    Calendar, Users, ShieldCheck, Plus, Info, FileText, CreditCard
} from "lucide-react";
...
const BookingSuccessPage = () => {
    ...
    const safeFormatDate = (dateStr) => {
        if (!dateStr) return "---";
        try {
            // Kiểm tra xem dateStr có phải định dạng dd/MM/yyyy không, nếu có thì trả về luôn
        ...
        <div className="min-h-screen bg-[#f8fafc] pb-20 font-sans antialiased">
            <div className="max-w-4xl mx-auto pt-12 px-4 text-center">
                ...
                <p className="text-slate-500 font-medium mb-8">
                    Đơn hàng đã được xác nhận. Mã đặt phòng đã được gửi đến <span className="text-slate-800 font-bold">{display.guestEmail}</span>
                </p>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl py-5 px-10 inline-flex items-center gap-12 shadow-sm mb-10">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-2xl font-black text-blue-700 tracking-tighter uppercase">
                            #{display.code}
                        </span>
                        <button
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <p className="font-bold text-slate-700">
                                    {safeFormatDate(display.checkIn)} — {safeFormatDate(display.checkOut)}
                                </p>
                                <p className="text-xs text-slate-500 font-bold uppercase tracking-tighter mt-1">
                                    {display.nights} đêm • {display.rooms} Phòng
                                </p>
                            </div>
                            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
                    <button className="flex flex-col items-center justify-center p-6 bg-[#2563eb] text-white rounded-2xl hover:bg-blue-700 transition-all group shadow-lg shadow-blue-100">
                        ...
                        <span className="font-black text-sm uppercase">Tải Voucher (PDF)</span>
