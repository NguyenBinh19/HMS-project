...
import { ChevronDown, ChevronUp, FileText, CheckCircle, Clock, AlertCircle, History, ExternalLink } from "lucide-react";
...
const DemoPayout = () => {
    ...
    const totalPaid = DEMO_PAYOUT_STATEMENTS.filter((s) => s.status === "PAID").reduce((sum, s) => sum + s.netPayout, 0);
    const totalPending = DEMO_PAYOUT_STATEMENTS.filter((s) => s.status !== "PAID").reduce((sum, s) => sum + s.netPayout, 0);
    ...
    // View Chi tiết
    if (selectedId) {
        const stmt = DEMO_PAYOUT_STATEMENTS.find((s) => s.id === selectedId);
                ...
                <div className="max-w-[1400px] mx-auto">
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl p-8 mb-6">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-xs text-slate-400 font-medium mt-1">Mã bảng kê: {stmt?.statementCode}</p>
                            </div>
                        ...
                        <div className="grid grid-cols-4 gap-6">
                            <div>
                                ...
                                <p className="text-lg font-black text-slate-900">{formatCurrency(DEMO_PAYOUT_DETAIL.grossRevenue)} <span className="text-xs text-slate-400">đ</span></p>
                            </div>
                            <div>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Hoa hồng nền tảng</p>
                                <p className="text-lg font-black text-red-500">-{formatCurrency(DEMO_PAYOUT_DETAIL.platformCommission)} <span className="text-xs text-slate-400">đ</span></p>
                            </div>
                            <div>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Thuế & Phí</p>
                                <p className="text-lg font-black text-red-500">-{formatCurrency(DEMO_PAYOUT_DETAIL.tax)} <span className="text-xs text-slate-400">đ</span></p>
                            </div>
                            <div>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Thực nhận</p>
                                <p className="text-lg font-black text-green-600">{formatCurrency(DEMO_PAYOUT_DETAIL.netPayout)} <span className="text-xs text-slate-400">đ</span></p>
                            </div>
                        </div>
                        ...
                        {stmt?.status === "PENDING_CONFIRMATION" && (
                            <div className="mt-6 pt-6 border-t border-slate-100 flex items-center justify-between">
                                ...
                                <button onClick={handleConfirm} className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-black hover:bg-blue-700 transition-colors">Xác nhận đối soát (Demo)</button>
                            </div>
                        )}
                    </div>
                    ...
                    {confirmed && (
                        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3 mb-6">
                            ...
                            <span className="text-sm font-bold text-green-700">Xác nhận đối soát thành công! (Bản Demo)</span>
                        </div>
                    )}
        ...
        <div className="min-h-screen bg-gray-50 p-6 font-sans text-gray-900">
            <div className="max-w-[1400px] mx-auto">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500">Theo dõi trạng thái thanh toán và chi tiết các kỳ đối soát (Kỳ: 26 tháng trước - 25 tháng này)</p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl p-6">
                        ...
                        <p className="text-2xl font-black text-slate-800">{DEMO_PAYOUT_STATEMENTS.length}</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden">
                    <div className="p-6 border-b border-slate-50">
                        <h3 className="font-black text-slate-800 flex items-center gap-2 uppercase text-[10px] tracking-widest">
                            <History size={16} className="text-blue-600" /> Tất cả các kỳ đối soát ({DEMO_PAYOUT_STATEMENTS.length})
                        </h3>
                    </div>
                    <table className="w-full text-left text-sm border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                <th className="px-8 py-4">Mã bảng kê</th>
                                <th className="px-8 py-4">Chu kỳ</th>
                            ...
                            {DEMO_PAYOUT_STATEMENTS.map((stmt) => {
                                ...
                                return (
                                    <tr key={stmt.id} className="hover:bg-slate-50/30 transition-colors group">
                                        ...
                                        <td className="px-8 py-4 font-bold text-slate-600">{stmt.totalBookings} đơn</td>
                                        ...
                                        <td className="px-8 py-4 text-right">
                                            <button onClick={() => setSelectedId(stmt.id)} className="text-slate-300 hover:text-blue-600 transition-colors" title="Xem chi tiết">
